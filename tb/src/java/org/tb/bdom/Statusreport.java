...
private static final long serialVersionUID = 3L;
	...
	public Byte getOverallStatus() {
		Byte status = 0;
		if (getTrendstatus() > status) status = getTrendstatus();
		if (getNeedforaction_status() > status) status = getNeedforaction_status();
		if (getAim_status() > status) status = getAim_status();
		if (getBudget_resources_date_status() > status) status = getBudget_resources_date_status();
		if (getRiskmonitoring_status() > status) status = getRiskmonitoring_status();
		if (getChangedirective_status() > status) status = getChangedirective_status();
		if (getCommunication_status() > status) status = getCommunication_status();
		if (getImprovement_status() > status) status = getImprovement_status();
		if (getMiscellaneous_status() > status) status = getMiscellaneous_status();
