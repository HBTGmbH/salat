...
public interface StatusreportRepository extends CrudRepository<Statusreport, Long> {
  ...
  Optional<LocalDate> getMaxUntilDateForCustomerOrderId(long id);
  ...
  List<Statusreport> getUnreleasedFinalStatusReports(long customerOrderId, long senderId, LocalDate date);

  @Query("select s from Statusreport s where s.sort = 1 and s.released is null and s.customerorder.id = :customerOrderId "
      + "and s.sender.id = :senderId and s.untildate > :date order by s.untildate desc")
  List<Statusreport> getUnreleasedPeriodicalStatusReports(long customerOrderId, long senderId, LocalDate date);
